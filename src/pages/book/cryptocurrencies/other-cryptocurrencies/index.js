...
const OtherCryptocurrencies = () => (
  <Page>
    <ChapterTitle>Other relevant cryptocurrencies (for payments)</ChapterTitle>
    <Paragraph>
      After Bitcoin made its appearance numerous other projects has been created
      inspired by Bitcoin.
    </Paragraph>
    ...
    <ul>
      ...
      <li>
        Different Proof of work algorithm (scrypt instead of Hashcash, in order
        to make it ASIC-resistant)
      </li>
    ...
    <Paragraph>
      ...
      transactions remain 100% unlinkable, private and untraceable even through
      blockchain analysis.
    </Paragraph>
    ...
    <Paragraph>
      ...
      payment providers that are interconnected through Ripple’s technology. The
      main services that RippleNet offers are:
    </Paragraph>
    <ul>
      <li>
    ...
    <Paragraph>
      ...
      other unit of value. The biggest selling point of XRP is that it is going
      to be used as a global currency inside RippleNet, solving problems like
      the need to maintain foreign currency deposits (<Italic>
        nostro accounts
      </Italic>).
    </Paragraph>
    <Paragraph>
      ...
      blockchain. This gives Ripple a large measure of control over XRP’s inner
      workings, which has led many to argue that it’s not truly decentralized.
    </Paragraph>
    <h5>Monetary policy</h5>
    <Paragraph>
      Ripple is a not-mineable, all the tokens were created when the blockchain
      was launched. Ripple currently controls over 50% of all tokens, giving the
      company a potential control over XRP market value.
    </Paragraph>
    <SubChapterTitle>IOTA (IOT)</SubChapterTitle>
    <Paragraph>
      IOTA is a cryptocurrencies focused on IoT (micro) payments. The demand of
      micropayments is growing together with the explosion of IoT and Blockchain
      like Bitcoin are currently far away from enabling instant and (almost)
      free transactions that requires by this market. IOTA does not uses a
      traditional blockchain, is built over a distributed ledger technology
      (tangle).Transactions are free and there are no miners, and no more blocks
      (see tangle in chapter 1).
    </Paragraph>
  </Page>
);
