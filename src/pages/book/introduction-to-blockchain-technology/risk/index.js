    ...
    <ul>
      <li>
        <Paragraph>
          ...
          anything that can be leveraged to perform an attack. Every system has
          vulnerabilities, some of them could be removed and some are intrinsic
          to the system (could be mitigated but not removed). Not all the
          vulnerabilities are exploitable. For example cryptographic algorithms
          are vulnerable to brute-force attacks, but this vulnerability is not
          exploitable right now since no one has enough computational power to
          perform a brute-force attack on such algorithms.
        </Paragraph>
      </li>
      <li>
        <Paragraph>
      ...
      <li>
        <Paragraph>
          <Bold>Threat</Bold>. Express all the potential factors that could lead
          to exploit a vulnerability. This value cannot be controlled since it
          comes from outside the system. Things that influences the threat level
          for example are: how skilled an attacker is, how many attackers there
          are, how motivated attackers are etc.
        </Paragraph>
      </li>
    </ul>
    <Paragraph>
      ...
      hundreds of billions of dollars into public addresses in a system that is
      constantly under attack. The real strength of a blockchain is in its
