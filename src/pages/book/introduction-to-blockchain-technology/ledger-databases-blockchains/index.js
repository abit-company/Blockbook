...
const LedgerDatabaseBlockchain = () => (
  <Page>
    ...
    <Important>A ledger is an item used for recording transactions.</Important>
    <Paragraph>
      ...
      completely digital. This means that everything that is saved in the ledger
      must be expressed in a digital form.
    </Paragraph>
    ...
    <Paragraph>
      ...
      concept. The purpose of both technologies is to store data but while with
      the former we can insert, delete and modify data, with a ledger we can
      only append new data. It is possible to use a database to implement a
      ledger by enforcing some constraints over the set of all possible
      operations. However, when talking about Blockchain, it’s better to use the
    ...
    <Paragraph>
      ...
      all, Blockchain technology is not going to replace databases. Some of the
      scenarios that now involve databases will be moved to Blockchains, but
    ...
    <Paragraph>
      ...
      and trustworthy individuals (whether that is a person, an organization or
      a machine). A Blockchain, on the other hand, can be used by unknown and
      untrusted parties, without the need of any form of access-control.
    ...
    <Paragraph>
      As we said before, at the heart of a Blockchain there is a digital ledger.
      The underlying structure of this ledger is one of the key characteristics
      of a Blockchain.
    </Paragraph>
    <Important>
      In a Blockchain, the digital ledger is structured as a chain of records,
      called blocks, each being responsible of storing some kind of information
      - from simple records of transactions to entire computer programs.
    </Important>
    ...
    <Important>
      ...
      cryptography, that ensures it follows in sequence from the previous block,
      resulting into a linear sequence which ultimately forms a chain. The first
    ...
    <Paragraph>
      ...
      blocks can have different sizes and store different types of information.
      The connection between blocks is enforced by the use of a hash function,
      which creates an unbreakable link between blocks.
    </Paragraph>
  </Page>
);
