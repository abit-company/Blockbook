  ...
  <Page>
    ...
    <Important>
      <ul>
        ...
        <li>
          Different nodes follow different rules yet sharing the same history of
          transactions (<Bold>soft fork</Bold>).
        </li>
      </ul>
    </Important>
    <Paragraph>
    ...
    <Paragraph>
      ...
      <cite>
        ...
        only-vegan restaurant then you are soft forking it. if you change it to
        include meat you are hard forking it.
      </cite>
    </Paragraph>
    <SubChapterTitle>Regular fork</SubChapterTitle>
    <Important>
      A regular fork is a temporary divergence in the state of a blockchain that
      occurs when two or more miners create a block at (nearly) the same time.
      Propagation time and network latencies cause the blockchain to temporary
      split into two different chains.
    </Important>
    <Paragraph>
      ...
      some miners. If two miners, miner A which in the USA and miner B which is
      in China, found a new block in a time window that is enough short to not
      allow the new block to be broadcasted to all the nodes in the network,
      there is a regular fork. All the miners close to miner A will acknowledge
      the fact the miner A find the new block and add this block to their
      blockchain, the miners close to miner B acknowledge the fact that miner B
      found the new block and add this block to their blockchain.
    </Paragraph>
    ...
    <Paragraph>
      ...
      nodes usually follow the longest chain. When the new block is found one of
      the two chain became the longest chain and all the nodes on the other
      chain merge to the longest chain.
    </Paragraph>
    {/* <ImageWrapper label="Longest chain">{longestChain}</ImageWrapper> */}
    <Paragraph>
      At T4 the chain is splitted. At T5 the miners on chain A find a new block
      and chain A became the longest chain. Nodes that have acknowledged miner’s
      B block realized that they are mining on the wrong chain and merge to
      chain A. That is the other reason is usually better to wait more than one
      confirmation
    </Paragraph>
    <SubChapterTitle>Soft fork</SubChapterTitle>
    <Paragraph>
      A soft fork is a change in the consensus rules of a blockchain where old
      rules are still valid. It is not mandatory for nodes to upgrade to the new
      rules. The blocks created with the new rules are recognized as valid by
      both the nodes that follow to the new rules and by the ones that follow
      the old rules.
    </Paragraph>
    <Important>
    ...
    <Paragraph>
      ...
      nodes. SegWit expanded the block size while remaining a soft fork. Without
      going into technical details, SegWit changed how the block size is
      calculated by giving a weight to each byte. A normal byte is assigned
      ...
      malleability, which is a requirement to create off chain transactions (e.g
      Lightning Network, we will explore that later).
    </Paragraph>
    ...
    <Paragraph>
      ...
      able to recognise those those blocks as valid. If all nodes update to the
      new consensus rules then there is no chain split, otherwise, If some node
      decide not to update it will cause a divergence in the state of the
    ...
    <Paragraph>
      ...
      of the Blockchain and every member who believe in this fork upgrade their
      systems, leaving the previous blockchain.
    </Paragraph>
    {/* <ImageWrapper label="Hard fork with chain split">
        {chainSplit}
      </ImageWrapper> */}
    <Paragraph>There could be many reason that lead to an hard fork:</Paragraph>
    <ul>
      <li>
        correct security risk that require an important change in the consensus
        rules.
      </li>
    ...
    <Paragraph>
      ...
      decided to hard-fork and undo the attack while the other part did not want
      to invalidate the immutability of the blockchain. The result was an hard
      fork that created two blockchains: Ethereum (ETH, the forked blockchain)
      and Ethereum Classic (ETC, the original blockchain).
    </Paragraph>
    <h5>Bitcoin Cash hard fork</h5>
    <Paragraph>
      Another example of disagreement in the community that lead to an hard fork
      is the creation of Bitcoin Cash, that was forked from Bitcoin. The
    ...
    <Paragraph>
      ...
      saying they will not update to mine the new blocks. Exchanges could remove
      a certain token or merchants could stop supporting that Blockchain in
      their products. Each group has its own ideas, but none of them can do
      anything without the consensus of the other groups. Developers could not
      propose an update if they don’t have miners consensus or users consensus,
      the same for all the other actors involved. This is a classic game theory
      problem where an equilibrium must be found in order to maintain trust in
      the system. A situation in which those actors can’t reach a consensus
      could cause a loss of trust in the Blockchain, since no one like
      uncertainty when it deals with money.
    </Paragraph>
  </Page>
);
